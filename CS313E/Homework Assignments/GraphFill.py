...
class ImageGraph:
    ...
    def bfs(self, start_index, color):
        ...
        only_color = self.nodes[start_index].color

        while not(node_queue.is_empty()):

            current_node = self.nodes[node_queue.dequeue()]
            if current_node.visited is False and current_node.color == only_color:

                current_node.visit_and_set_color(color)
                ImageGraph.print_image(self)
    ...
    def dfs(self, start_index, color):
        ...
        node_stack = Stack()
        node_stack.push(start_index)
        only_color = self.nodes[start_index].color


        while not(node_stack.is_empty()):

            current_node = self.nodes[node_stack.pop()]
            if current_node.visited is False and current_node.color == only_color:

                current_node.visit_and_set_color(color)
                for i in reversed(current_node.edges):
